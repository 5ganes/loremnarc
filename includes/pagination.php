<?php
function Pagination($content, $type, $limit=10, $alias_name,$lan)
{
	global $pagename;
  ...
  if (($curpage-1) > 0){
   $pageList .= "<a href='".$lan.'/'.$alias_name.'/'."page/".($curpage-1)."' title='Previous Page' class='paging'>&laquo; Prev</a>&nbsp;";
  }
	...
	for($i=1; $i<=$pages; $i++)
	{
		if($curpage == $i)
			...
			$pageList .= ' <a href="'.$lan.'/'.$alias_name.'/'.'page/'. $i .'" class="paging">'. $i . '</a> ';
	}
	
  if (($curpage+1) <= $pages){
   $pageList .= "<a href='".$lan.'/'.$alias_name.'/'."page/".($curpage+1)."' title='Next Page' class='paging'>&nbsp;Next &raquo;</a>";
  }
  $pageList .= "</div>\n";
